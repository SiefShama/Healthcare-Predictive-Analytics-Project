    ...
    for name, file_id in model_files.items():
        ...
        if not os.path.exists(model_path):
            try:
                ...
                st.error(f"Error downloading model {name}: {e}")
...
# --- Mode Toggle ---
mode = st.sidebar.radio("Choose View", ["📊 Plots", "🤖 ML Predictions"])
...
if mode == "📊 Plots":
    ...
    st.subheader("General Health Ratings")
    st.bar_chart(data['GenHlth'].value_counts())
else:
    ...
    if not model:
        ...
        if st.button("Predict"):
            input_data = pd.DataFrame([[  # Prepare input data for prediction
